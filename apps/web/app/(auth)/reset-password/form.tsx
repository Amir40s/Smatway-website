      ...
      <div className="w-full animate-fade-in-up">
        ...
        <div className="mb-8">
          ...
          <h1 className="font-[var(--font-display)] text-4xl font-semibold leading-[1.05] tracking-tight text-zinc-950">
            {t("Link")} <span className="italic text-red-700">{t("expired.")}</span>
          </h1>
          <p className="mt-2 text-[14px] text-zinc-500">{t("This reset link is missing or invalid. Request a new one below.")}</p>
        </div>
        ...
        >
          <span>{t("Request new reset link")}</span>
          <ArrowRightIcon />
        </Link>
      </div>
    );
    ...
    <div className="w-full animate-fade-in-up">
      ...
      <div className="mb-8">
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-emerald-300/60 bg-white/70 px-3 py-1 backdrop-blur">
          <span className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-700">{t("New password")}</span>
        </div>
        <h1 className="mt-4 font-[var(--font-display)] text-4xl font-semibold leading-[1.05] tracking-tight text-zinc-950">
          {t("Create a")} <span className="italic text-emerald-700">{t("new password.")}</span>
        </h1>
        <p className="mt-2 text-[14px] text-zinc-500">{t("Choose something you can remember, but nobody else could guess.")}</p>
      </div>
      ...
      <div className="rounded-3xl border border-zinc-200/80 bg-white p-6 sm:p-7 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_10px_32px_-12px_rgba(15,23,42,0.08)]">
        ...
        {success ? (
          <div className="animate-scale-in text-center py-4">
            ...
            <div className="font-[var(--font-display)] text-xl font-semibold text-zinc-950">{t("Password updated")}</div>
            <p className="mt-2 text-[13px] leading-relaxed text-zinc-500">{t("Redirecting you to sign in…")}</p>
          </div>
        ) : (
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div>
              <label htmlFor="password" className="mb-1.5 block text-[12px] font-semibold uppercase tracking-[0.1em] text-zinc-700">{t("New password")}</label>
