...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    async function fetchPayments() {
      try {
        ...
        setError(err?.message || t("Failed to load payments"));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="max-w-4xl space-y-5">
      <div className="mb-2">
        <h1 className="text-xl font-semibold tracking-tight text-zinc-900">{t("Payments")}</h1>
        <p className="text-sm text-slate-400 mt-0.5">{t("View your payment history and transactions.")}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 text-center">
          ...
          <h3 className="text-sm font-semibold text-zinc-900 mb-1">{t("No payments yet")}</h3>
          <p className="text-xs text-slate-500">{t("Your payment history will appear here once travelers complete their payments.")}</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500">
                <tr>
                  <th className="px-5 py-4 font-semibold">{t("Booking ID")}</th>
                  <th className="px-5 py-4 font-semibold">{t("Traveler")}</th>
                  <th className="px-5 py-4 font-semibold">{t("Route")}</th>
                  <th className="px-5 py-4 font-semibold">{t("Date")}</th>
                  <th className="px-5 py-4 font-semibold text-right">{t("Amount")}</th>
                  <th className="px-5 py-4 font-semibold text-center">{t("Status")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {payments.map((booking) => (
                  <tr key={booking.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-5 py-4 text-zinc-900">{booking.traveler?.name || t("Unknown")}</td>
                    ...
                    <td className="px-5 py-4 text-center">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                        {t(booking.paymentStatus === "PAID" ? "PAID" : "COMPLETED")}
                      </span>
                    </td>
                  </tr>
                ))}
