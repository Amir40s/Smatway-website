...
export default function RateJourneyPage() {
  const t = useT();
  return (
    <Suspense fallback={<div className="py-20 text-center text-sm text-slate-400">{t("Loading rating form...")}</div>}>
      <RateJourneyForm />
    </Suspense>
  );
}
...
function RateJourneyForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedBookingId) {
      setError(t("Please select a booking reference to rate."));
      return;
    }
    setLoading(true);
    setError(null);
    try {
      ...
      setError(err.message || t("Failed to submit feedback. Please try again."));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <h1 className="text-3xl font-bold text-slate-900 mb-2">{t("Thank You!")}</h1>
        <p className="text-slate-600 max-w-md">{t("Your feedback has been sent to our team. We appreciate your time to help us improve SmatWay.")}</p>
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
      <div className="mb-10 text-center">
        ...
        <h1 className="text-3xl font-bold text-slate-900">{t("Rate the Journey")}</h1>
        {selectedBookingId && (
          <p className="text-xs font-semibold text-emerald-600 bg-emerald-50 inline-block px-3 py-1 rounded-full mt-2 animate-pulse">
            {t("Rating for Booking Ref:")} #{selectedBookingId.slice(0, 8).toUpperCase()}
          </p>
        )}
        <p className="text-slate-500 mt-2">{t("Help us maintain the highest standards by sharing your experience.")}</p>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-5 mb-6 space-y-3">
        <label className="block text-xs font-bold text-zinc-700 uppercase tracking-wider">
          {t("Select Booking Reference to Rate")}
        </label>
        {loadingBookings ? (
          <p className="text-xs text-slate-400">{t("Loading your recent bookings...")}</p>
        ) : bookings.length === 0 ? (
          <p className="text-xs text-slate-400">{t("No bookings found to rate.")}</p>
        ) : (
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
        ...
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("1. How punctual was the vehicle/train?")}</Label>
          <div className="flex flex-wrap gap-4">
            {["Very Punctual", "Punctual", "Late", "Very Late"].map(opt => (
              <label key={opt} className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-slate-700">{t(opt)}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        {/* Q2 */}
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("2. How clean was the inside and outside?")}</Label>
          <div className="flex flex-wrap gap-4">
        ...
        {/* Q3 */}
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("3. How friendly was the driver?")}</Label>
          <div className="flex flex-wrap gap-4">
        ...
        {/* Q4 */}
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("4. How safe did you feel?")}</Label>
          <div className="flex flex-wrap gap-4">
        ...
        {/* Q5 */}
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("5. Was there any harassment on the way? (If yes, explain below)")}</Label>
          <textarea 
            name="harassment" 
            onChange={handleChange} 
            placeholder={t("Type 'No' or explain what happened...")}
        ...
        {/* Q6 */}
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("6. Was the journey interrupted by law enforcement agencies and where?")}</Label>
          <textarea 
            name="lawEnforcementInterruption" 
            onChange={handleChange} 
            placeholder={t("e.g., No, or Yes at Checkpoint X...")}
            className="w-full p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 min-h-[100px] outline-none transition-all"
            required
          />
        </div>
        ...
        {/* Q7 */}
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("7. Anything unusual you would like to share with us?")}</Label>
          <textarea 
            name="unusualEvents" 
            onChange={handleChange} 
            placeholder={t("Any other unusual events...")}
            className="w-full p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 min-h-[100px] outline-none transition-all"
          />
        </div>
        ...
        {/* Q8 */}
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("8. How good was the service?")}</Label>
          <div className="flex flex-wrap gap-4">
        ...
        {/* Q9 */}
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("9. Did you notice the driver collecting cash from passengers?")}</Label>
        ...
        {/* Q10 */}
        <div className="space-y-4">
          <Label className="text-base font-semibold">{t("10. Were the seats good and did you sit comfortable? Did you notice overload and where?")}</Label>
          <textarea 
            name="seatComfortAndOverload" 
            onChange={handleChange} 
            placeholder={t("Tell us about the seats and any overloading...")}
            className="w-full p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 min-h-[100px] outline-none transition-all"
            required
          />
        </div>
